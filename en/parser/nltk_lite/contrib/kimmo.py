...
class KimmoGUI:
    ...
    def validate(self, *args):
        ...
        for l in self.kimmoinstance.validateLexicon:
            if l not in self.kimmoinstance.validateAlternations:
                if l:
        ...
        for a in self.kimmoinstance.validateAlternations:
            if a not in self.kimmoinstance.validateLexicon:
                if a:
...
class KimmoFSARule:
    def __init__(self, name, pair_description, state_descriptions):
        ...
        for index, is_final, next_state_array in state_descriptions:
            if not (is_final or is_final == False):
                raise ValueError(
    ...
    def is_state(self, index):
        return index in self.transitions
            ...
            for i, pair, size1, size2 in self.sorted_pairs(
                ...
                if pair.matches(input, output, subsets):
                    ...
                    if 0:  # ('@' in pair.__repr__()):
                        ...
                        if self.transitions[index][i] in self.transitions:
        ...
        if position >= len(input_tokens):  # and (not morphological_state)
            ...
            for pair_input, pair_output in self._pair_alphabet:
                ...
                for s in rule_states:
                    ...
                    if contains_halt_state and isinstance(rule, KimmoArrowRule):
                ...
                for rule in self.rules():
                    ...
                    if required_truth_value is None:
                        continue
