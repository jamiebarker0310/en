...
class Synset:
            ...
            def extractVerbFrames(index, vfTuples):
                return tuple(
                    [
                        ...
                        for t in list(
                            filter(
                                lambda t, i=index: string.atoi(t[2], 16) in (0, i),
                                vfTuples,
                            )
                        )
                    ]
    ...
    def getPointers(self, pointerType=None):
        ...
        if pointerType is None:
            ...
            return list(
                filter(
                    lambda pointer, type=pointerType: pointer.type == type,
                    self._pointers,
                )
            )
    ...
    def __str__(self):
        ...
        return (
            ...
            + self.pos
            + ": "
            + string.joinfields([sense.form for sense in self.getSenses()], ", ")
            + "}"
        )
